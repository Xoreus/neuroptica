...
class ONN_Simulation:
    ...
    def plotAll(self, cmap='gist_heat', trainingLoss=0.00):
        ...
        tick_fmt = '%.2f'
        PT_subset_x = 0.6 # use this to rescale plots accordingly when simulated a subset of (0., 1, 40)
        PT_subset_y = 0.6
        LPU_subset_x = 0.85
        LPU_subset_y = 0.6
        ...
        plt.figure(figsize=(14*LPU_subset_x, 10*LPU_subset_y)) # compress the graph (around) quarter in size, by cutting top half and compress horizontally
        plt.pcolor(self.loss_dB, self.phase_uncert_theta, self.accuracy_LPU, vmin=100/(self.N+1)*0, vmax=100, cmap=cmap, rasterized=True)
        ...
        # cs = plt.contour(self.loss_dB, self.phase_uncert_theta[:len(self.phase_uncert_theta)//2], self.accuracy_LPU[:self.accuracy_LPU.shape[0]//2,:], [self.zeta*np.max(self.accuracy_LPU)], colors=[contour_color2], linewidths=contour_linewidth)
        proxy = [plt.Rectangle((0,0),1,1,fc = contour_color2)]
        plt.legend(proxy, [f'FoM={self.LPU_FoM:.5f}'], prop={'size': legend_size})
        ...
        plt.figure(figsize=(14*PT_subset_x, 10*PT_subset_y)) # cut the graph (around) quarter in size
        ...
        # cs = plt.contour(self.phase_uncert_theta[:len(self.phase_uncert_theta)//2], self.phase_uncert_theta[:len(self.phase_uncert_theta)//2], self.accuracy_PT[:self.accuracy_PT.shape[0]//2,:self.accuracy_PT.shape[1]//2], [self.zeta*np.max(self.accuracy_PT)], colors=[contour_color2], linewidths=contour_linewidth)
        proxy = [plt.Rectangle((0,0),1,1,fc = contour_color2)]
        plt.legend(proxy, [f'FoM={self.PT_FoM:.5f}'], prop={'size': legend_size})
