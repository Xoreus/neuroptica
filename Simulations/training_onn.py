...
def train_single_onn(onn, model, loss_function='cce'): # cce: categorical cross entropy, mse: mean square error
    ...
    elif loss_function == 'ccew':
        print(f"beta penalize factor = {onn.beta}")
        optimizer = neu.InSituBinaryAdamWeighted(model, neu.CategoricalCrossEntropyWeighted, step_size=onn.STEP_SIZE, pos_weight=[1, onn.beta])
