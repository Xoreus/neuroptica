    ...
    for onn.topo in onn_topo:
        for lossDiff in loss_diff:
            for trainLoss in training_loss:
                ...
                # df.loc[temp] = [onn.topo, test_number+1] + accuracy
                temp_df[trainLoss] = list(accuracy_dict)
        if onn.topo == 'Diamond':
            temp_df["Mesh"] = "Diamond"
        elif onn.topo == 'Clements':
            temp_df["Mesh"] = "Clements"
        else:
            temp_df["Mesh"] = "Reck"
        temp_df["Test Number"] = list(np.linspace(1, onn.max_number_of_tests, onn.max_number_of_tests))
