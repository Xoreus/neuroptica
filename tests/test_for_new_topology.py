...
X, y, Xt, yt = blob_maker(targets=int((N+2)/2), features=int((N+2)/2), nsamples=1000)
X = np.array([list(np.zeros(int((N-2)/2))) + list(samples) for samples in X])
Xt = np.array([list(np.zeros(int((N-2)/2))) + list(samples) for samples in Xt])
