...
def init_onn_settings():
    ...
    onn.BATCH_SIZE = 2**6 
    onn.EPOCHS = 600
    onn.STEP_SIZE = 0.0015 # Learning Rate
    onn.SAMPLES = 60 # Per Class
...
def dataset(onn, dataset='MNIST', half_square_length=2):
    ...
    if dataset == 'Gauss':
        ...
        onn.X, onn.y, onn.Xt, onn.yt = create_datasets.FFT_MNIST_PCA(classes=classes, features=features, nsamples=onn.SAMPLES) # this gives real valued vectors as input samples
    return onn

def normalize_dataset(onn, normalization='None', experimental=False):
