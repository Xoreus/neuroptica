...
function plotAcc_SingleLossAllModelsAllTrainings(FOLDER)
...
for ii = 1:length(loss_dB_train([1,end]))
    for jj = 1:length(phase_uncert_train([1,end]))
        for l_idx = 1:length(loss_dB_test([1,2,end]))
            figure
            for model_idx = 1:length(Models)
                ...
                if ~contains(Models{model_idx}, 'N')
                    ...
                    plot(phase_uncert_test, Model_acc(:, l_idx), 'linewidth', 2)
                    hold on
                end
            end
            legend(legend_);
