...
for ii = 1:length(SimulationSettings.ONN_Setups)
    ...
    ylabel('Losses (MSE)', 'fontsize', 1.5*fontsz, 'interpreter','latex')
    ...
    ylabel('Accuracy', 'fontsize', 1.5*fontsz, 'interpreter','latex')
    ylim([0 100])
    a = get(gca,'XTickLabel');
    set(gca,'XTickLabel',a,'FontName','Times','fontsize',fontsz)
    a = get(gca,'YTickLabel');
    ...
    title(sprintf('Loss and Training/Validation Accuracy\n ONN model with %s topology\nDataset: %s\n Loss/MZI = %.3f dB with a standard deviation of %s dB\n Phase Uncertainty = %.3f Radians',...
        ...
        SimulationSettings.loss_diff, SimulationSettings.phase_uncert_theta(1)), 'fontsize', 1.5*fontsz, 'interpreter','latex')
