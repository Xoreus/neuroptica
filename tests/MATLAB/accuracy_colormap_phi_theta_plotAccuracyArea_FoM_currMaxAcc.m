...
function accuracy_colormap_phi_theta_plotAccuracyArea_FoM_currMaxAcc(FOLDER, SimulationSettings, fig_of_merit_value, showContour)
fontsz = 38;
...
for model_idx = 1:length(SimulationSettings.ONN_Setups)
    ...
    for kk = 1:length(colormaps)
        ...
        for loss_idx = 1:size(accuracy, 3)
            ...
            savefig([FOLDER, sprintf('Matlab_Figs/ColorMap-FigureOfMerit-Model=%s_Loss=%.3f_FoM=%.3f_cmap=%s-currMaxAcc.fig', SimulationSettings.ONN_Setups{model_idx}, ...
                SimulationSettings.loss_dB(loss_idx), fig_of_merit_value, colormaps{kk})])
            saveas(gcf, [FOLDER, sprintf('Matlab_Pngs/ColorMap-FigureOfMerit-Model=%s_Loss=%.3f_FoM=%.3f_cmap=%s-currMaxAcc.png', SimulationSettings.ONN_Setups{model_idx}, ...
                SimulationSettings.loss_dB(loss_idx), fig_of_merit_value, colormaps{kk})])
            close(gcf)
        end
    end
