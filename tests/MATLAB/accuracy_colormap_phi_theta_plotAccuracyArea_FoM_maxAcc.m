...
%                 SimulationSettings.loss_dB(loss_idx), str2double(SimulationSettings.loss_diff), area_of_merit), 'fontsize', fontsz, 'interpreter','latex')
            ...
            savefig([FOLDER, sprintf('Matlab_Figs/ColorMap-AreaOfMerit-Model=%s_Loss=%.3f_FoM=%.3f_cmap=%s-totalMaxAcc.fig', SimulationSettings.ONN_Setups{model_idx}, ...
                SimulationSettings.loss_dB(loss_idx), fig_of_merit_value, colormaps{kk})])
            saveas(gcf, [FOLDER, sprintf('Matlab_Pngs/ColorMap-AreaOfMerit-Model=%s_Loss=%.3f_FoM=%.3f_cmap=%s-totalMaxAcc.png', SimulationSettings.ONN_Setups{model_idx}, ...
                SimulationSettings.loss_dB(loss_idx), fig_of_merit_value, colormaps{kk})])
            close(gcf)
        end
    end
