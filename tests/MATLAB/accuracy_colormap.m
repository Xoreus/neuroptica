...
for model_idx = 1:length(SimulationSettings.ONN_Setups)
    ...
    savefig([FOLDER, sprintf('Matlab_Figs/ColorMap-Model=%s_Loss=[%.3f-%.2f]_Colormap=%s.fig', SimulationSettings.ONN_Setups{model_idx}, ...
        min(SimulationSettings.loss_dB), max(SimulationSettings.loss_dB), colormaps{ii})])
    saveas(gcf, [FOLDER, sprintf('Matlab_Pngs/ColorMap-Model=%s_Loss=[%.3f-%.2f]_Colormap=%s.png', SimulationSettings.ONN_Setups{model_idx}, ...
        min(SimulationSettings.loss_dB), max(SimulationSettings.loss_dB), colormaps{ii})])
    end
end
