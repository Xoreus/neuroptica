...
losses_dB = np.linspace(0,2,2)# in dB
iterations = 500 # number of times to retry same loss
...
for idx in [4]:#, 0, 1, 3, 2]: 
    ...
    losses, trn_accuracy, val_accuracy = optimizer.fit(X.T, y.T, Xt.T, yt.T, epochs=150, batch_size=2**5, show_progress=True)
    ...
    for loss in losses_dB:
        ...
        for phase_uncert in phase_uncerts:
            print(f'loss = {loss} dB, Phase Uncert = {phase_uncert:.2f}')
            if idx == 0:
                model = neu.Sequential([
